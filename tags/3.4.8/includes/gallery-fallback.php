...
add_action( 'acf/init', 'uqbhi_fallback_gallery_register_value_filters' );

/**
 * Register the ACF value filters that expose the native gallery to get_field().
 *
 * These MUST NOT run when ACF Pro's gallery field type is present: they would
 * rewrite the stored attachment IDs into attachment arrays at `load_value`,
 * and ACF Pro's own gallery `format_value` then chokes on the arrays and
 * returns an empty gallery — breaking the feed (property emitted without the
 * mandatory `<multimidia>`, so the portal finalises every ad) and the gallery
 * field in the editor.
 *
 * The check therefore has to happen *after* ACF has registered its field
 * types. It is deferred to `acf/init` for exactly that reason: running it at
 * file-load time (on `plugins_loaded`) is too early — the `gallery` type is not
 * registered yet, `uqbhi_has_acf_pro_gallery()` wrongly returns false, and the
 * filters get attached on a site that does have Pro.
 */
function uqbhi_fallback_gallery_register_value_filters() {
