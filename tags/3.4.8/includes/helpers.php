...
/**
 * Read a gallery-type field (galeria_de_imagens / plantas) resiliently.
 *
 * ACF Pro's `gallery` field only returns a value while its field type is
 * registered. When Pro is inactive — or the type is not yet available at the
 * moment `uqbhi_register_acf_fields()` runs, in which case the field is removed
 * via `acf_remove_local_field()` — `get_field()` yields null/empty even though
 * the attachment IDs are still stored in post meta. The feed would then emit
 * the property without a `<multimidia>` block, which the portal rejects,
 * finalising every ad (and the property also fails the ">= 5 fotos" check and
 * drops out of the feed entirely).
 *
 * Falling back to the raw meta — the same source the native metabox and the
 * Elementor tag already use — keeps the gallery readable regardless of ACF
 * Pro's state. Items are normalised to ACF Pro's array shape so downstream
 * consumers (feed, validation) work unchanged.
 *
 * @param int    $post_id  Property post ID.
 * @param string $meta_key 'galeria_de_imagens' or 'plantas'.
 * @return array Array of ACF-style attachment arrays. Empty when none exist.
 */
function uqbhi_get_gallery_items( $post_id, $meta_key ) {
	$value = function_exists( 'get_field' ) ? get_field( $meta_key, $post_id ) : null;
	if ( is_array( $value ) && ! empty( $value ) ) {
		return $value;
	}

	// ACF returned nothing usable — recover from the raw attachment IDs.
	$raw = get_post_meta( $post_id, $meta_key, true );
	if ( function_exists( 'uqbhi_fallback_gallery_normalize_items' ) ) {
		return uqbhi_fallback_gallery_normalize_items( $raw );
	}
...
function uqbhi_validate_imovel( $post_id ) {
	...
	$gallery   = uqbhi_get_gallery_items( $post_id, 'galeria_de_imagens' );
